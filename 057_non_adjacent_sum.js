...
const nonAdjacentSum = (nums, hash={}) => {
    // todo
     // todo
    if(nums in hash ) return hash[nums]
    if(nums.length == 0) return 0
    
    const include = nums[0] + nonAdjacentSum(nums.slice(2),hash)
    const exclude =  nonAdjacentSum(nums.slice(1),hash)
    hash[nums] = Math.max(include,exclude)
    return hash[nums]
  };
